...
def get_properties_for_app(request: Request, session: Session = Depends(get_session)):
    """
    قانون امنیت مطلق حریم خصوصی:
    ۱. فایل‌های عمومی آژانس (is_exclusive == False) برای همه مشاورین و مدیران قابل رویت است.
    ۲. فایل‌های شخصی (is_exclusive == True) فقط برای مشاوری که آن را ثبت کرده (created_by_id == user.id) نمایش داده می‌شود (حتی مدیرعامل دسترسی ندارد).
    """
    ...
    # کوئری ایزوله‌سازی فایل‌های شخصی
    props = session.exec(
        select(Property)
        ...
        .where(
            or_(
                Property.is_exclusive == False, # فایل‌های عمومی آژانس
                Property.created_by_id == user.id # فایل‌های شخصی فقط و فقط سازنده آن
            )
        )
