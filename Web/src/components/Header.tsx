...
import { Bell, ChevronDown, Sun, LogOut } from 'lucide-react';
      ...
      <div className="flex items-center gap-2 lg:gap-3">
            ...
            <>
              ...
              <div className="absolute left-0 mt-2 w-80 bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl shadow-black/50 z-40 overflow-hidden animate-fade-in-up">
                <div className="px-4 py-3 border-b border-slate-700 flex items-center justify-between">
                  ...
                  <span className="text-[11px] px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-400">۳ جدید</span>
                </div>
                <div className="max-h-72 overflow-y-auto">
                  {notifications.map((n) => (
                    <div key={n.id} className="flex items-start gap-3 px-4 py-3 hover:bg-slate-700/50 transition-colors border-b border-slate-700/50 last:border-0">
        ...
        <div className="relative">
          <button 
            ...
            className="flex items-center gap-2.5 pr-1 pl-2 py-1 rounded-xl hover:bg-slate-800 transition-colors"
          >
            ...
            <div className="hidden lg:block text-right">
              <p className="text-sm font-semibold text-slate-200 leading-tight">رضا محمدی</p>
          ...
          {profileOpen && (
            <>
              ...
              <div className="absolute left-0 mt-2 w-48 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-2 z-40 animate-fade-in-up">
                <button 
                  onClick={() => { logout(); setProfileOpen(false); }}
                  className="w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-bold text-rose-400 hover:bg-rose-500/10 transition-colors"
                >
